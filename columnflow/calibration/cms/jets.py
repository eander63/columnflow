...
@calibrator(
    ...
    propagate_met=True,
    # name of the MET collection to propagate to, and its raw counterpart.
    # Run 3 analyses using PUPPI jets should derive with
    # met_name="PuppiMET", raw_met_name="RawPuppiMET".
    met_name="MET",
    raw_met_name="RawMET",
...
def jec(
    ...
    *get_jec_config* can be adapted in a subclass in case it is stored differently in the config.
    ...
    def correct_jets(pt, area, eta, phi, rho, evaluator_key="jec", run=None):
        ...
        # run-dependent data correctors (e.g. 2023) require the run number,
        # broadcast from per-event to per-jet layout
        if run is not None:
            variable_map["run"] = ak.values_astype(
                ak.broadcast_arrays(run, pt)[0], np.float32,
            )
    ...
    if self.propagate_met:
        ...
        jec_factors_subset_type1_met = correct_jets(
            ...
            run=(events.run if self.dataset_inst.is_data else None),
        )
...
def jec_init(self: Calibrator) -> None:
    ...
    # add MET variables
    if self.propagate_met:
        self.uses |= {f"{self.raw_met_name}.pt", f"{self.raw_met_name}.phi"}
...
@calibrator(
    ...
    # toggle for propagation to MET
    propagate_met=True,
    # name of the MET collection to propagate to (see jec above)
    met_name="MET",
    # only run on mc
    mc_only=True,
    # function to determine the correction file
    ...
    *get_jer_config* can be adapted in a subclass in case it is stored differently in the config.
    ...
    if self.propagate_met:
        ...
        events = set_ak_column_f32(
            events, f"{self.met_name}.pt_unsmeared", events[self.met_name].pt)
        events = set_ak_column_f32(
            events, f"{self.met_name}.phi_unsmeared", events[self.met_name].phi)
...
def jer_init(self: Calibrator) -> None:
    ...
    self.uses |= {
        f"{self.met_name}.pt", f"{self.met_name}.phi",
    }
    self.produces |= {
        f"{self.met_name}.pt", f"{self.met_name}.phi",
        f"{self.met_name}.pt_jer_up", f"{self.met_name}.pt_jer_down",
        f"{self.met_name}.phi_jer_up", f"{self.met_name}.phi_jer_down",
        f"{self.met_name}.pt_unsmeared", f"{self.met_name}.phi_unsmeared",
    }
