...
def electron_weights(
    ...
    *get_electron_config* can be adapted in a subclass in case it is stored differently in the
    ...
    for syst, postfix in [
        ...
        args = (self.year, syst, self.wp, sc_eta, pt)
        if len(self.electron_sf_corrector.inputs) == 6:
            args += (phi,)
        sf_flat = self.electron_sf_corrector(*args)
...
def electron_weights_setup(
    ...
    # check versions
    if self.electron_sf_corrector.version not in (2, 3, 4, 5):
        raise Exception(
